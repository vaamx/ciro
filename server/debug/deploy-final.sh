#!/bin/bash
# Final deployment script for the backend with proper volume handling
...
# Basic info
echo "Deploying fixed backend (final version)"
...
docker build -t ciro-backend-final .
...
# Tag the image
IMAGE_TAG="final-$(date +%Y%m%d-%H%M%S)"
...
docker tag ciro-backend-final $BACKEND_IMAGE_URI
