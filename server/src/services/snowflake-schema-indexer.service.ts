...
import { RowLevelIndexerService } from './data-storage/row-level-indexer.service';
...
export class SnowflakeSchemaIndexerService {
  ...
  private static instance: SnowflakeSchemaIndexerService | null = null;
  ...
  private rowLevelIndexerService: RowLevelIndexerService;
  ...
  private constructor(
    ...
    if (!fs.existsSync(this.METADATA_CACHE_DIR)) {
      fs.mkdirSync(this.METADATA_CACHE_DIR, { recursive: true });
      this.logger.info(`Created schema metadata cache directory: ${this.METADATA_CACHE_DIR}`);
    }
    
    // Ensure Snowflake knowledge directory exists
    if (!fs.existsSync(this.SNOWFLAKE_KNOWLEDGE_DIR)) {
      fs.mkdirSync(this.SNOWFLAKE_KNOWLEDGE_DIR, { recursive: true });
      this.logger.info(`Created Snowflake knowledge directory: ${this.SNOWFLAKE_KNOWLEDGE_DIR}`);
    }
    
    // Initialize WebSocket service for broadcasting updates
    this.websocketService = new WebSocketService();
    
    // Initialize RowLevelIndexerService for row-level indexing
    this.rowLevelIndexerService = RowLevelIndexerService.getInstance();
  }
  
  /**
   * Get singleton instance
   */
  public static getInstance(): SnowflakeSchemaIndexerService {
    if (!SnowflakeSchemaIndexerService.instance) {
      const snowflakeService = SnowflakeService.getInstance();
      const openaiService = OpenAIService.getInstance();
      const qdrantService = QdrantService.getInstance();
      const chunkingService = ChunkingService.getInstance();
      
      SnowflakeSchemaIndexerService.instance = new SnowflakeSchemaIndexerService(
        snowflakeService,
        openaiService,
        qdrantService,
        chunkingService
    ...
    return SnowflakeSchemaIndexerService.instance;
  }
  ...
  private async getTableRowCount(
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(`Approximate count failed, falling back to exact count: ${approxError.message}`);
      }
  ...
  private async generateEmbeddings(
    ...
    try {
      ...
      try {
        ...
        this.logger.info(`Deleted existing collection ${collectionName}`);
      } catch (error) {
        // Ignore errors if collection doesn't exist
      }
      
      // Create a new collection for this schema
      const vectorSize = 1536; // Default for text-embedding-ada-002
...
${table.columns.map(c => `${c.name} (${c.type})${c.primaryKey ? ' [PRIMARY KEY]' : ''}${c.foreignKey ? ` [FOREIGN KEY to ${c.foreignKey.table}.${c.foreignKey.column}]` : ''}: ${c.description || this.inferColumnDescription(c.name, c.type)}`).join('\n')}`;
              ...
              if (tableDescription.trim().length > 10) {
                ...
                // Create embeddings for columns (with batching if there are many columns)
                const columnBatchSize = 20; // Process 20 columns at a time
                for (let j = 0; j < table.columns.length; j += columnBatchSize) {
                  const columnBatch = table.columns.slice(j, j + columnBatchSize);
                  
                  // Prepare column descriptions for batch embedding
                  const columnDescriptions: string[] = [];
                  const columnMetadata: Array<{name: string, type: string}> = [];
                  
                  columnBatch.forEach(column => {
                    const columnDescription = this.createColumnDescription(table, column);
                    if (columnDescription.trim().length > 10) {
                      columnDescriptions.push(columnDescription);
                      columnMetadata.push({
                        name: column.name,
                        type: column.type
                      });
                  ...
                  if (columnDescriptions.length > 0) {
                    // Create embeddings for all columns in one batch API call
                    try {
                      const columnEmbeddingVectors = await this.openaiService.createEmbeddings(
                        columnDescriptions
                      );
                      
                      // Add column embeddings with their metadata
                      columnEmbeddingVectors.forEach((vector, idx) => {
                        embeddings.push({
                          id: `${dataSourceId}:${database}:${schema}:${table.tableName}:${columnMetadata[idx].name}`,
                          text: columnDescriptions[idx],
                          type: 'column',
                          vector: vector,
                          metadata: {
                            dataSourceId,
                            ...
                            schema,
                            table: table.tableName,
                            column: columnMetadata[idx].name,
                            dataType: columnMetadata[idx].type,
                            lastUpdated: new Date()
                          }
                        });
                      });
                    } catch (error: any) {
                      this.logger.warn(`Failed to create embeddings for columns in ${table.tableName}: ${error.message}`);
                      // Continue processing other column batches
                    }
                  }
                }
              }
...
`;
  ...
  private async updateTableEmbeddings(
    ...
    try {
      ...
      this.logger.info(`Updated ${embeddings.length} embeddings for ${database}.${schema}.${tableName}`);
  ...
  private async processLargeTableForEmbeddings(
    ...
    try {
      ...
      const { EnhancedMetadataService } = await import('../services/metadata-extraction/enhanced-metadata-service');
        ...
        try {
          ...
          embeddings.push({
            ...
            metadata: {
              ...
              dateRanges: enhancedMetadata.dateRanges,
              lastUpdated: new Date()
            }
          });
