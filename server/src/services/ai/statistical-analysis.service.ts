...
export class StatisticalAnalysisService {
   ...
   */
  public detectTrend(data: number[]): TrendResult | null {
    if (data.length < 5) {
  ...
  public generateForecast(
    ...
    if (method === ForecastMethod.MOVING_AVERAGE) {
      ...
      mape = this.calculateMAPE(data.slice(-windowSize), Array(windowSize).fill(ma)) || 0;
      rmse = this.calculateRMSE(data.slice(-windowSize), Array(windowSize).fill(ma)) || 0;
      ...
      mape = this.calculateMAPE(data, oneStepAheadForecasts) || 0;
      rmse = this.calculateRMSE(data, oneStepAheadForecasts) || 0;
      ...
      // Calculate performance metrics
      mape = this.calculateMAPE(data, predicted) || 0;
      rmse = this.calculateRMSE(data, predicted) || 0;
    }
  ...
  public generateInsights(data: Record<string, number[]>): StatisticalInsight[] {
    ...
    variables.forEach(variable => {
      ...
      if (stats && stats.skewness !== undefined && Math.abs(stats.skewness) > 1) {
        insights.push({
          id: uuidv4(),
          type: 'general',
          ...
          insights.push({
            ...
            description: `Based on historical patterns, ${variable} is projected to ${
              forecast.forecastData[forecast.forecastData.length - 1] > values[values.length - 1]
                ? 'increase'
                : 'decrease'
            } in the future, with a forecast error (MAPE) of ${((forecast.mape || 0) * 100).toFixed(1)}%.`,
            confidence: 0.95 - ((forecast.mape || 0) * 0.5), // Lower confidence for higher error
            importance: 0.85,
            relatedVariables: [variable],
            visualizationRecommendation: {
              type: 'line',
