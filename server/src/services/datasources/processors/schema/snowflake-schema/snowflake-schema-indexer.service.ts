...
import { EnhancedMetadataService } from '../enhanced-metadata-extraction.service';
...
${this.createTableDescription(chunkTableMetadata, true)}`;
  ...
  private async getDataSourceCredentials(_dataSourceId: string): Promise<SnowflakeCredentials> {
    // In a real scenario, you would fetch credentials securely based on dataSourceId
    // For this example, we'll use environment variables or a mock
    // Simulating fetching credentials
    this.logger.debug(`Fetching credentials (using mock/env for dataSourceId: ${_dataSourceId})`);
    return {
      ...
      warehouse: process.env.SNOWFLAKE_WAREHOUSE || 'mock_warehouse',
      database: process.env.SNOWFLAKE_DATABASE || 'mock_database',
      schema: process.env.SNOWFLAKE_SCHEMA || 'mock_schema',
    };
  }
} 