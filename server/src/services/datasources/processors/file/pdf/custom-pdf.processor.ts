    ...
    private async extractTextWithOcr(filePath: string): Promise<string> {
        ...
        try {
            ...
            try {
                ...
                this.logger.error(`pdftoppm command failed: ${pdfError instanceof Error ? pdfError.message : String(pdfError)}`);
                // Clean up temp directory before throwing
                try {
                    fs.rmdirSync(tempDir, { recursive: true });
                } catch (cleanupError) {
                    this.logger.warn(`Error cleaning up temp directory after pdftoppm failure: ${cleanupError}`);
                }
                throw new Error(`Failed to convert PDF to images using pdftoppm: ${pdfError instanceof Error ? pdfError.message : 'Unknown error'}`);
            }
    ...
    async processFile(
        ...
        try {
            ...
            if (elements.length === 0 && rawText.trim().length > 0) {
                 ...
                 const textChunks = await this.chunkingService.chunkText(rawText, { chunkSize: 1000, overlap: 100 }); // Example parameters
