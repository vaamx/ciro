...
export class RagService {
  ...
  /**
   * Select model for query if OpenAIService doesn't have this method
  ...
  async processQuery(
    ...
    try {
      ...
      return this.createErrorResponse(
        `I encountered an error while processing your request: ${error instanceof Error ? getErrorMessage(error) : 'Unknown error'}. Please try again.`,
        'processing_error',
        startTime,
  ...
  private createErrorResponse(
    ...
    if (error) {
      this.logger.error(`Error processing query: ${error instanceof Error ? getErrorMessage(error) : 'Unknown error'}`);
      if (error.stack) {
    ...
    return {
      ...
      metadata: {
        ...
        error: errorCode,
        errorMessage: error instanceof Error ? getErrorMessage(error) : (error ? String(error) : undefined)
...
Please provide a detailed answer based only on this information. If the information doesn't contain what I need, please say so clearly.`;
    ...
    try {
      ...
      this.logger.error(`Error generating response: ${error instanceof Error ? getErrorMessage(error) : 'Unknown error'}`);
      return { 
  ...
  private async processUniversalQuery(
    ...
    try {
      ...
      if (sources.length === 0 && allSources.length > 0) {
        ...
        const filterWithLowerThreshold = async (q: string, srcs: any[]): Promise<any[]> => {
          try {
            ...
            const embeddings = await this.openaiService.createEmbeddings(prepareEmbeddingInput(texts));
      ...
      this.logger.error(`Error in universal query processing: ${getErrorMessage(error)}`);
  ...
  private async filterRelevantSourcesEnhanced(query: string, sources: any[]): Promise<any[]> {
    ...
    try {
      ...
      this.logger.error(`Error filtering relevant sources: ${getErrorMessage(error)}`);
  ...
  private async processDocumentSummaryQuery(
    ...
    try {
      ...
      try {
        ...
        const selectedModel = this.selectModelForQuery(prompt);
        const messages = [
          { 
            id: `system-${Date.now()}`,
            role: 'system' as 'system' | 'user' | 'assistant' | 'error',
            content: systemPrompt,
            timestamp: Date.now(),
            status: 'complete' as 'complete' | 'streaming' | 'error' | 'loading'
          },
          { 
            id: `user-${Date.now()}`,
            role: 'user' as 'system' | 'user' | 'assistant' | 'error',
            content: prompt,
            timestamp: Date.now(),
            status: 'complete' as 'complete' | 'streaming' | 'error' | 'loading'
          }
  ...
  private async processEntityCountQuery(
    ...
    try {
      ...
      // Select a model for the completion
      const modelName = this.selectModelForQuery(userPrompt);
...
Format your response with a multi-step analysis approach even though you don't have specific data.`;
      ...
      try {
        ...
        this.logger.error(`Error generating completion: ${getErrorMessage(error)}`);
        content = "I'm sorry, but I couldn't find specific information to answer your question. Could you try rephrasing your query or providing more details about what you're looking for?";
      }
      ...
      this.logger.error(`Error generating fallback response: ${getErrorMessage(secondaryError)}`);
      return {
        sources: [],
  ...
  private async getDataSourcesInfo(dataSourceIds: string[] | number[]): Promise<Array<{id: string | number, name: string, type: string}>> {
    try {
      ...
      this.logger.error(`Error getting data source info: ${getErrorMessage(error)}`);
      return [];
    }
  }
  ...
  private async tryGetNumericCollectionName(collectionName: string): Promise<string> {
    try {
      ...
      this.logger.error(`Error converting collection name: ${error instanceof Error ? getErrorMessage(error) : String(error)}`);
      return collectionName;
    }
  }
...
Include a summary of your findings at the end.`;
    ...
    try {
      ...
      return {
        sources: allDocuments,
        content: `I encountered an error while analyzing the data: ${getErrorMessage(error)}`,
        model: modelToUse
      };
    }
  }
  ...
  async listDataSources(organizationId: string): Promise<Array<{
    ...
    try {
      ...
      try {
        ...
        this.logger.error(`Error listing collections: ${getErrorMessage(error)}`);
      ...
      this.logger.error(`Error listing data sources: ${getErrorMessage(error)}`);
  ...
  private extractTablesFromMarkdown(content: string): any[] {
    ...
    while ((match = tableRegex.exec(content)) !== null) {
      try {
        ...
        this.logger.error(`Error extracting table from markdown: ${getErrorMessage(err)}`);
      }
    }
...
Please provide a detailed answer based only on this information. If the information doesn't contain what I need, please say so clearly.`;
      ...
      const response = await this.openaiService.generateChatCompletion(
        [
          ...
          createChatMessage('user', userPrompt)
        ],
        { model: model }
      );
      ...
      this.logger.error(`Error generating response: ${error instanceof Error ? getErrorMessage(error) : 'Unknown error'}`);
      return { 
        content: `I encountered an error while processing your request: ${error instanceof Error ? getErrorMessage(error) : 'Unknown error'}. Please try again.` 
      };
    }
  }
...
Format your output as a JSON object with these fields:
{
  ...
  "summary": "One paragraph summary of the key points"
      ...
      const structuredResponseObj = await this.openaiService.generateChatCompletion(
        [
          createChatMessage('system', 'You are an analytical assistant that breaks down information into structured steps.'),
          createChatMessage('user', structurePrompt)
        ],
        { model: "gpt-4-turbo" }
      );
      ...
      try {
        ...
        this.logger.error(`Error parsing structured analysis: ${error instanceof Error ? getErrorMessage(error) : 'Unknown error'}`);
      }
      
      return undefined;
    } catch (error) {
      this.logger.error(`Error generating structured analysis: ${error instanceof Error ? getErrorMessage(error) : 'Unknown error'}`);
      return undefined;
    }
  }
  ...
  private extractInsightsFromContent(content: string): string[] {
    try {
      ...
      this.logger.error(`Error extracting insights: ${error instanceof Error ? getErrorMessage(error) : 'Unknown error'}`);
      return [];
    }
  }
...
// Define DataSourceType as enum for proper usage as values
enum DataSourceType {
  EXCEL = 'excel',
  PDF = 'pdf',
  DOC = 'doc',
...
/**
 * Helper to create a properly formatted ChatMessage
 */
function createChatMessage(role: 'system' | 'user' | 'assistant' | 'error', content: string): ChatMessage {
  return {
    id: `${role}-${Date.now()}`,
    role,
    content,
...
/**
 * Helper to convert array of strings to a single string for embeddings
 */
function prepareEmbeddingInput(input: string | string[]): string {
