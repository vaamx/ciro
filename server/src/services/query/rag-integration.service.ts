...
export class RAGIntegrationService {
  ...
  async executeRAGQuery(
    ...
    try {
      ...
      throw new Error(`Failed to execute hybrid search: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  private buildEnhancedContext(
    ...
    queryResult.results.forEach((result: any, index: number) => {
  ...
  private async generateAnswer(
    ...
    // Convert ChatCompletionMessageParam[] to ChatMessage[] for OpenAI service
    const formattedMessages = messages.map(msg => ({
      id: uuidv4(), // Assuming uuidv4 is imported or add an import for it
    ...
    // Parse the response since it's a standard Response object
    const responseText = await completion.text();
    let responseData;
    try {
      responseData = JSON.parse(responseText);
      return responseData.content || 'No answer generated';
    } catch (error) {
