...
export class QueryStrategySelector {
  ...
  private buildFilterConditions(classification: QueryClassification): any {
    ...
    const logger = createServiceLogger('QueryStrategySelector');
    
    // No need to build filter conditions if we don't have classification
    if (!classification) {
      return conditions;
    }
    
    // Check if entities object is a key-value map
    if (classification.entities && typeof classification.entities === 'object' && !Array.isArray(classification.entities)) {
      for (const [entityType, entityValue] of Object.entries(classification.entities)) {
        if (entityValue) {
          conditions[entityType] = entityValue;
        }
      }
    } else if (classification.entities && Array.isArray(classification.entities)) {
      // Handle case where entities might be an array
      classification.entities.forEach((entity: any) => {
