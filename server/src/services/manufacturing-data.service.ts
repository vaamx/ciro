        ...
        for (let i = 0; i < chunks.length; i += batchSize) {
          ...
          try {
            ...
            const embeddings = await Promise.all(
              batch.map(chunk => this.openaiService.createEmbeddings(chunk).then(emb => emb[0]))
            );
  ...
  async enhancedManufacturingQuery(
    ...
    try {
      ...
      const ragResult = await this.ragService.processQuery(
        ...
        dataSourceIds,
        sessionId,
        8 // More context for manufacturing queries (as number)
      );
  ...
  private extractStatisticsFromSources(sources: any[]): Record<string, any> {
    ...
    for (const source of sources) {
      ...
      if (numericMatches && numericMatches.length > 5) {
        const values = numericMatches.map(Number).filter((n: number) => !isNaN(n));
