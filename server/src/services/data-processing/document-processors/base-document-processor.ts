...
/**
 * We use QdrantAdapter to maintain compatibility with existing code
 */
class QdrantAdapter {
  private qdrantClient: QdrantClientService;
  private logger: winston.Logger;

  constructor(qdrantClient: QdrantClientService, private readonly eventManager: EventManager, private readonly qdrantClientService: QdrantClientService) {
    ...
    this.logger = winston.createLogger({
      ...
      format: winston.format.combine(
        ...
        winston.format.printf((info) => {
          ...
          const formattedMessage = `${timestamp} [${level.toUpperCase()}] [QdrantAdapter]: ${message}`;
  ...
  async createCollection(collectionName: string, options: any): Promise<void> {
    try {
      const client = this.qdrantClient.getClient();
...
export abstract class BaseDocumentProcessor {
  ...
  protected qdrantService: any; // Using any to avoid type issues
  ...
  constructor(serviceName: string) {
    ...
    // Initialize adapter with the QdrantClientService
