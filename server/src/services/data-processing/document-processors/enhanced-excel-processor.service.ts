...
import { DataSourceStatus } from './base-document-processor';
      ...
      try {
        ...
        for (const sheetName of sheetsToProcess) {
          ...
          try {
            ...
            let units: Array<{ column: string; unit: string }> = [];
        ...
        for (let i = 0; i < chunksToEmbed.length; i += batchSize) {
          ...
          try {
            ...
            const embeddings = await this.createEmbeddings(batchTexts);
            ...
            const points = batch.map((chunk, index) => ({
              ...
              vector: embeddings[index][0], // Extract the first embedding vector from the array
              payload: {
                text: chunk.text,
