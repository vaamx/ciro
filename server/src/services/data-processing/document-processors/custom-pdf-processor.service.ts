...
import { WebSocketService as OldWebSocketService } from '../websocket.service';
...
export class CustomPdfProcessorService extends BaseDocumentProcessor {
    ...
    constructor(
        ...
        protected qdrantService: QdrantClientService,
        private readonly websocketService?: WebSocketService
    , private readonly chunkingService: ChunkingService, private readonly openAIService: OpenAIService, private readonly qdrantClientService: QdrantClientService) {
    ...
    private async extractTextWithOcr(filePath: string): Promise<string> {
        ...
        try {
            ...
            try {
                ...
                this.logger.warn(`pdftoppm failed, trying pdf-poppler: ${pdfError instanceof Error ? pdfError.message : String(pdfError)}`);
        ...
        try {
            ...
            if (dataSourceId && dataSourceId.includes('-')) {
                try {
                    ...
                    this.logger.error(`Error finding numeric data source ID: ${error instanceof Error ? error.message : String(error)}`);
                }
            ...
            if (numericDataSourceId) {
                try {
                    await this.updateDataSourceStatus(this.ensureString(numericDataSourceId), 'processing', {});
                } catch (error) {
            ...
            if (!pdfExists) {
                ...
                if (numericDataSourceId) {
                    await this.updateDataSourceStatus(this.ensureString(numericDataSourceId), 'error', { error: 'File not found' });
                }
                return { 
                    status: 'error', 
            ...
            try {
                ...
                await this.updateDataSourceStatus(this.ensureString(numericDataSourceId), 'error', {
            ...
            return this.processExtractedElements(elements, this.ensureString(numericDataSourceId), collectionName, metadata);
        ...
        try {
            ...
            if (dataSourceId.includes('-')) {
                ...
                try {
                    ...
                    this.logger.debug(`Error querying file_to_data_source table: ${mappingError instanceof Error ? mappingError.message : String(mappingError)}`);
                }
                ...
                if (!foundId) {
                    try {
                        ...
                        this.logger.debug(`Error querying data_sources metadata: ${metadataError instanceof Error ? metadataError.message : String(metadataError)}`);
                    }
                }
                ...
                if (!foundId) {
                    ...
                    try {
                        ...
                        this.logger.debug(`Error finding data source by UUID in name/description: ${error instanceof Error ? error.message : String(error)}`);
                    }
                }
