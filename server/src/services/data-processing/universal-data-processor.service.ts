...
export class UniversalDataProcessorService {
  ...
  constructor(
    ...
    this.statisticalAnalysis = this.statisticalAnalysisService;
  ...
  public async processDataQuery(
    ...
    if (completion) {
      ...
      const responseBody = await completion.json() as { content?: string };
        ...
        while ((stepMatch = stepRegex.exec(stepsContent)) !== null) {
          ...
          if (stepType === 'chart' || stepType === 'visualization') {
            ...
            const vizType = this.extractVisualizationType(stepContent) || 
                           (suggestedVisualizationType !== VisualizationType.NONE ? suggestedVisualizationType : 'bar_chart');
            
            // Generate visualization config with safe string handling
            step.visualization = {
              type: vizType as string,
              config: this.generateVisualizationConfig(stepData, vizType as string, String(stepDescription || 'Visualization'))
            };
          }
...
Return ONLY the visualization type name, nothing else.`;
   ...
   */
  private generateVisualizationConfig(data: any, type: string, title: string | undefined): any {
