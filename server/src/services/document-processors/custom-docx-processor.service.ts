...
export class CustomDocxProcessorService extends BaseDocumentProcessor {
    ...
    async processFile(
        ...
        try {
            ...
            this.logger.error(`Error processing DOCX file: ${error.message}`);
            
            // Update data source status to error
            if (!isTestUpload && !skipDatabaseCheck) {
                await this.updateDataSourceStatus(dataSourceId, 'error', error.message);
            }
        ...
        const result = await mammoth.convertToHtml({ path: filePath });
            ...
            if (!result.value || result.value.length < 50) {
                ...
                try {
                    ...
                    this.logger.error(`Error in alternative extraction: ${extractError.message}`);
                }
            }
            ...
            this.logger.error(`Error extracting elements from DOCX: ${error.message}`);
    ...
    private parseHtmlContent(html: string, fileName: string): any[] {
        ...
        // Process lists
        $('ul, ol').each((listIndex, listElement) => {
    ...
    private async processExtractedElements(
        ...
        return {
            ...
            this.logger.error(`Error in vector processing: ${error.message}`);
            return {
                chunks,
                vectorsUpserted: 0,
                error: error.message
        ...
        try {
            ...
            if (!(await qdrantService.collectionExists(collectionName))) {
                ...
                try {
                    ...
                    this.logger.error(`Error creating collection: ${createError.message}`);
                }
            }
            ...
            for (let i = 0; i < processableChunks.length; i += dynamicBatchSize) {
                ...
                try {
                    ...
                    const embeddings = await this.openAIService.createEmbeddings(chunksContent, {
                        skipCache: true,
                    ...
                    if (embeddings.length !== batchWithContent.length) {
                        this.logger.warn(`Mismatch between chunks (${batchWithContent.length}) and embeddings (${embeddings.length})`);
                    }
                    ...
                    try {
                        ...
                        this.logger.error(`Error verifying vector storage: ${verifyError.message}`);
                    }
                } catch (batchError) {
                    this.logger.error(`Error processing batch: ${batchError.message}`);
