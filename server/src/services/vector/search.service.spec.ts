...
describe('QdrantSearchService', () => {
  ...
  describe('delete', () => {
    ...
    const normalizedCollectionName = 'delete_col'; // For expectation
    const pointIds = ['p1', 'p2'];

    beforeEach(() => {
        jest.spyOn(service, 'collectionExists').mockResolvedValue(true);
        // Ensure the specific method used by the service (delete) is mocked on the client instance
        mockQdrantClient.delete.mockReset().mockResolvedValue({ status: 'ok' } as any); 
    });

    it('should call client.delete if collection exists and IDs are provided', async () => {
      jest.spyOn(service, 'collectionExists').mockResolvedValueOnce(true);
      mockQdrantClient.delete.mockResolvedValueOnce({ status: 'ok' } as any); // Mock delete

      await service.delete(collectionName, pointIds);
      // Expect normalized name in delete call
      expect(mockQdrantClient.delete).toHaveBeenCalledWith(normalizedCollectionName, {
        points: pointIds,
      });
    });

    it('should not call client.delete if collection does not exist', async () => {
      jest.spyOn(service, 'collectionExists').mockResolvedValueOnce(false);
      mockQdrantClient.delete.mockClear(); // Clear mock from previous tests
    ...
    it('should not call client.delete if no point IDs are provided', async () => {
      jest.spyOn(service, 'collectionExists').mockResolvedValueOnce(true);
      mockQdrantClient.delete.mockClear();

      await service.delete(collectionName, []); // Empty point IDs
      expect(mockQdrantClient.delete).not.toHaveBeenCalled();
      expect(createServiceLogger('QdrantSearchService').warn).toHaveBeenCalledWith(
    ...
    it('should throw error if qdrant client is unavailable when deleting', async () => {
      (mockQdrantClientService.getClient as jest.Mock).mockReturnValue(null);
       await expect(service.delete(collectionName, pointIds)).rejects.toThrow('Qdrant client is not available');
    });

    it('should re-throw on client.delete error', async () => {
        jest.spyOn(service, 'collectionExists').mockResolvedValueOnce(true);
        mockQdrantClient.delete.mockRejectedValue(new Error('Delete failed')); // Mock delete rejection
        await expect(service.delete(collectionName, pointIds)).rejects.toThrow('Delete failed');
    });
  });

  // Add tests for other methods like hybridSearch if they are part of QdrantSearchService
  // and not BaseSearchService, or if their Qdrant-specific logic needs testing.
}); 