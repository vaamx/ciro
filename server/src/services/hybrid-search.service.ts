...
export class HybridSearchService {
  ...
  private extractEntitiesFromConversation(conversationHistory: any[]): string[] {
    ...
    for (const message of conversationHistory) {
      ...
      for (const match of Array.from(matches)) {
        if (match && Array.isArray(match) && match[1] && typeof match[1] === 'string' && match[1].length > 2) {
  ...
  private enhanceQueryWithContext(query: string, conversationHistory: any[]): string {
    ...
    if (pronounPattern.test(query) && conversationHistory.length > 0) {
      ...
      if (matches.length > 0) {
        // Type assertion for the array of matches
        const lastMatch = matches[matches.length - 1] as RegExpMatchArray;
        
        // Check that we have a valid match with at least one capture group
        if (lastMatch && lastMatch.length > 1 && typeof lastMatch[1] === 'string') {
          // Use the last mentioned entity to resolve pronouns
          const entity = lastMatch[1];
