...
export async function redirectRagQueries(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const isRagQueryEndpoint = req.method === 'POST' && req.path === '/query';
    const isChatEndpoint = req.method === 'POST' && req.path.includes('/messages');
 ...
 */
async function findPossibleCollections(dataSourceIds: any[], qdrantService: any): Promise<Array<{originalId: any, collectionName: string, hasData?: boolean}>> {
  const result = [];
  ...
  for (const id of dataSourceIds) {
    ...
    if (specialMappings[idStr]) {
      for (const pattern of specialMappings[idStr]) {
  ...
  result.sort((a, b) => {
    ...
    const aIsSpecial = specialMappings[a.originalId]?.includes(a.collectionName) || false;
    const bIsSpecial = specialMappings[b.originalId]?.includes(b.collectionName) || false;
 ...
 */
function getCollectionPatterns(id: any): string[] {
  const patterns = [];
    ...
    const documents = searchResults.map(result => {
      try {
        ...
        if (!docContent) {
          ...
          const findStringFields = (obj, prefix = '') => {
