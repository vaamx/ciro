...
export const EnhancedStepByStepVisualization: React.FC<EnhancedStepByStepVisualizationProps> = ({
  ...
  const [addingToDashboard] = useState<string | null>(null);
  ...
  const [isReady, setIsReady] = useState<boolean>(false); // Add a ready state to prevent premature rendering
  ...
  useEffect(() => {
    // Set showMockData to false and never allow it to change
  ...
  }, [currentStep, steps.length, autoPlay, autoPlayDelay, onComplete, onStepChange]);
  ...
  const handleAddToDashboard = useCallback(async (visualization: any) => {
  ...
  if ((!steps || steps.length === 0) && insights.length === 0 && !summary && !visualizationData) {
    return (
  ...
  // New helper function to get final chart type for visualization
  const getFinalChartType = () => {
    if (!visualizationData) return 'bar';
    
    // Check if URL has pie chart parameter
    try {
      ...
      if (forcePieChart) {
        console.log('[CRITICAL DEBUG] Forcing pie chart from URL parameter');
        return 'pie';
      }
    } catch (err) {
    ...
    // Now that we've added the properties to the interface, we can safely access them
    return visualizationData.preserveType 
  ...
  const shouldRenderContent = isReady || initialSteps.length > 0 || summary || visualizationData;
            ...
            <div className="visualization-container bg-white dark:bg-gray-800/90 rounded-lg p-5 border border-gray-200 dark:border-gray-700/80 my-4 shadow-sm">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">
                  {(visualizationData && (visualizationData as any).preserveType && visualizationData.type) ? 
                    visualizationData.type : 
                    validateVisualizationType(visualizationData?.type)}
                </div>
              </div>
              ...
              <div className="h-96 w-full">
                <Suspense fallback={<VisualizationLoader />}>
                  {/* Determine the final chart type */}
                  {(() => {
                    // Determine the final chart type
                    const finalChartType = getFinalChartType();
                    
                    // Create a complete config object with options for the visualization
                    const vizConfig = {
                      ...visualizationData,
                      type: finalChartType,
                      options: {
                        ...(visualizationData?.options || {}),
                        // If type is pie, add special pie chart options
                        ...(finalChartType === 'pie' ? {
                          forceChartType: 'pie',
                          isPieChart: true,
              ...
              <div className="flex justify-end mt-5">
                <button
                  ...
                  onClick={() => {
                    ...
                    handleAddToDashboard(formattedVisualization);
                  }}
